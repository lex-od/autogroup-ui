...
import type { ApiConfiguration } from '@/types/admin.types';
...
const ApiConfigurationManagement: FC<ApiConfigurationManagementProps> = ({
  onCreateConfig,
  onEditConfig,
  className = '',
}) => {
  ...
  const { mutate: deleteConfig, isPending: isDeleting } = useDeleteApiConfiguration();
  const { mutate: testConnection, isPending: isTesting } = useTestApiConnection();
  ...
  const getServiceColor = (serviceName: string) => {
    switch (serviceName.toLowerCase()) {
      ...
      case 'assemblyai': return 'bg-purple-100 text-purple-800';
  ...
  const stats = {
    ...
    recentlyUsed: configs.filter(c => {
      ...
      const diffHours = (new Date().getTime() - new Date(c.last_used_at).getTime()) / (1000 * 60 * 60);
      return diffHours < 24;
    }).length,
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Использованы сегодня</CardTitle>
            <Activity className="h-4 w-4 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{stats.recentlyUsed}</div>
          </CardContent>
        </Card>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Настройка API-ключей для внешних сервисов (OpenAI, Groq, AssemblyAI, Gemini)
              </CardDescription>
            </div>
            {onCreateConfig && (
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <div className="text-gray-500 mb-2">API конфигурации не найдены</div>
                ...
                <Card key={config.id} className="hover:shadow-md transition-shadow">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        {/* Иконка сервиса */}
                        <div className={`h-12 w-12 rounded-lg flex items-center justify-center text-xl ${getServiceColor(config.service_name)}`}>
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <code className="text-sm bg-gray-100 px-2 py-1 rounded font-mono">
                              {visibleKeys[config.id] ? config.api_key : maskApiKey(config.api_key)}
                            </code>
                            <Button
                              variant="ghost"
                              size="sm"
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-500">
                            <span>Вызовов: {config.usage_count.toLocaleString('ru-RU')}</span>
                            <span>•</span>
                            <span>Последнее использование: {getLastUsedText(config.last_used_at)}</span>
                          </div>
                    ...
                    {config.configuration && Object.keys(config.configuration).length > 0 && (
                      <div className="mt-4 pt-4 border-t">
                        <div className="text-sm text-gray-600 mb-2">Конфигурация:</div>
                    ...
                    {/* Предупреждения */}
                    {!config.is_active && (
                      <div className="mt-4 flex items-center gap-2 text-orange-600 bg-orange-50 rounded p-2">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                Вы уверены, что хотите удалить эту API конфигурацию? 
                Это может нарушить работу связанных сервисов.
              </CardDescription>
            </CardHeader>
            <CardContent>
