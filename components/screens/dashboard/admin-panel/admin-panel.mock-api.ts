...
export const apiConfigurationsApi = {
  ...
  async testConnection(is: string): Promise<boolean> {
    // TODO: Реализовать через Edge Function
...
export const promptConfigurationsApi = {
  ...
  async update(
    ...
    const versionIncrement =
      data.content && data.content !== mockPrompts[promptIndex].content ? 1 : 0;
...
export const systemSettingsApi = {
  ...
  async updateSetting(
    ...
    for (const group of mockSystemSettings) {
      const settingIndex = group.settings.findIndex(
        (s) => s.setting_key === key,
      );
      if (settingIndex !== -1) {
        group.settings[settingIndex] = {
          ...
          description:
            data.description || group.settings[settingIndex].description,
          updated_at: new Date().toISOString(),
        };
...
export const logsApi = {
  ...
  async getProcessLogs(filters?: LogFilters): Promise<ProcessLog[]> {
    ...
    return mockProcessLogs.filter((log) => {
      if (filters?.service_name && log.service_name !== filters.service_name)
        return false;
