...
export const adminKeys = {
  ...
  processLogs: (filters?: LogFilters) =>
    [...adminKeys.logs(), 'process', filters] as const,
  transcriptionLogs: (filters?: LogFilters) =>
    [...adminKeys.logs(), 'transcription', filters] as const,
  auditLogs: (filters?: AuditLogFilters) =>
    [...adminKeys.logs(), 'audit', filters] as const,
...
export function useCreateApiConfiguration() {
  ...
  return useMutation({
    mutationFn: (data: CreateApiConfigData) =>
      apiConfigurationsApi.create(data),
    onSuccess: (newConfig) => {
      queryClient.invalidateQueries({
        queryKey: adminKeys.apiConfigs(),
      });
...
export function useUpdateApiConfiguration() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedConfig) => {
      ...
      toast.success(
        `API конфигурация "${updatedConfig.service_name}" обновлена`,
      );
    },
    onError: (error: Error) => {
...
export function useCreatePromptConfiguration() {
  ...
  return useMutation({
    mutationFn: (data: CreatePromptData) =>
      promptConfigurationsApi.create(data),
    onSuccess: (newPrompt) => {
      queryClient.invalidateQueries({
        queryKey: adminKeys.prompts(),
      });
...
export function useUpdatePromptConfiguration() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedPrompt) => {
      ...
      toast.success(
        `Промпт "${updatedPrompt.name}" обновлен (версия ${updatedPrompt.version})`,
      );
    },
    onError: (error: Error) => {
